...
- **実装予定エンドポイント**:
  - ⬜️ `POST /api/v1/permissions` - 権限作成
  - ⬜️ `GET /api/v1/permissions` - 権限一覧
  - ⬜️ `GET /api/v1/permissions/:id` - 権限詳細
  - ⬜️ `PUT /api/v1/permissions/:id` - 権限更新
  - ⬜️ `DELETE /api/v1/permissions/:id` - 権限削除
  - ⬜️ `GET /api/v1/permissions/matrix` - 権限マトリックス
  - ⬜️ `GET /api/v1/permissions/modules/:module` - モジュール別権限
  - ⬜️ `GET /api/v1/permissions/:id/roles` - 権限を持つロール一覧
...
#### **入力値検証**
- ⬜️ **モジュール名**: `binding:"required,min=2,max=50,alphanum"`
- ⬜️ **アクション名**: `binding:"required,min=2,max=50,alphanum"`
- ⬜️ **説明**: `binding:"omitempty,max=255"`
- ⬜️ **権限コード**: 自動生成（module:action形式）

#### **ビジネスルール**
- ⬜️ **Module・Action組み合わせ**: 有効な組み合わせの定義・検証
- ⬜️ **権限重複チェック**: module+actionの一意性確保
- ⬜️ **削除前チェック**: ロール割り当て・システム権限の確認
- ⬜️ **システム権限保護**: 基本権限（user:read等）の保護
- ⬜️ **権限階層検証**: 前提権限の存在確認

#### **セキュリティ**
- ⬜️ **認証必須**: 全エンドポイントでJWT認証が必要
- ⬜️ **権限チェック**: 権限管理権限（permission:*）の確認
- ⬜️ **監査ログ**: 全操作でリクエストユーザー・IP記録
- ⬜️ **システム権限制御**: 基本権限の変更・削除防止

### **データ操作機能**

#### **CRUD操作**
- ⬜️ **作成**: モジュール・アクション検証付き権限作成
- ⬜️ **取得**: 関連ロール・使用状況を含む詳細取得
- ⬜️ **更新**: 説明・アクション名の変更対応
- ⬜️ **削除**: 安全な削除（依存関係チェック）

#### **権限マトリックス**
- ⬜️ **マトリックス表示**: モジュール×アクションの2次元表示
- ⬜️ **ロール別権限**: ロール毎の権限保有状況表示
- ⬜️ **権限カバレッジ**: 未使用権限・孤立権限の検出
- ⬜️ **権限統計**: モジュール別・アクション別の使用統計

#### **フィルタリング・検索**
- ⬜️ **モジュール別フィルタ**: `module`パラメータ
- ⬜️ **アクション別フィルタ**: `action`パラメータ
- ⬜️ **ロール使用フィルタ**: `used_by_role`パラメータ
- ⬜️ **テキスト検索**: `search`パラメータ（説明・モジュール・アクション）
- ⬜️ **ページング**: `page`・`limit`パラメータ
- ⬜️ **ソート**: モジュール順・アクション順・作成日順
